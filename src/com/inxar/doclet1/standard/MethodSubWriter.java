...
public class MethodSubWriter extends ExecutableMemberSubWriter {
	...
	while (iterator.hasNext()) {
	    MethodDoc member = (MethodDoc)iterator.next();
	    if (!member.isStatic())
		iterator.remove();
	}
	...
	if (!members.isEmpty()) {
	    Collections.sort(members);
            printSummaryHeader("doclet.Class_Method_Summary", classdoc);
            for (int i = 0; i < members.size(); ++i) {
                MethodDoc member = (MethodDoc)members.get(i);
		if (!member.isStatic())
		    continue;
	...
	if (!members.isEmpty()) {
	    Collections.sort(members);
            printSummaryHeader("doclet.Instance_Method_Summary", classdoc);
            for (int i = 0; i < members.size(); ++i) {
                MethodDoc member = (MethodDoc)members.get(i);
		if (member.isStatic())
		    continue;
...
//          writer.anchor("method_detail");
//          writer.bold(writer.getText("doclet.Method_Detail"));
    }
